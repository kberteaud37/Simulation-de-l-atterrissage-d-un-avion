...
def main():
    ...
    if st.session_state.step == 1:
        ...
        if selected_code:
            ...
            coord_aeroport = airports_df[airports_df["ident"] == selected_code][["latitude_deg", "longitude_deg"]].iloc[
                0]
    ...
    # === ÉTAPE 2 : Affichage météo ===
        ...
        try:
            ...
            if pluie and glace:
                st.error(
                    "⚠️ ATTENTION CONDITIONS EXTRÊMES : Conditions glacées avec pluie – Coefficient de friction minimal.")
            elif glace:
        ...
        if selected_code == "Autre":
            ...
            cl_max = st.number_input("Coefficient de portance max à l’atterrissage", min_value=0.0, step=0.1,
                                     format="%.2f")
            cd_train = st.number_input("Coefficient de trainée du train d’atterrissage", min_value=0.0, step=0.01,
                                       format="%.3f")
        ...
        st.success(
            f"✈️ **Distance d'atterrissage au sol** : {(avion.calcul_S_B() + avion.calcul_S_FR() + avion.calcul_S_TR()) * coef_secu:.2f} ft")
